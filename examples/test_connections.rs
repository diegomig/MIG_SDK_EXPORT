...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let database_url = env::var("DATABASE_URL").unwrap_or_else(|_| {
        "postgresql://mig_topology_user:mig_topology_pass@localhost:5432/mig_topology".to_string()
    });
            ...
            match sqlx::query("SELECT version()").fetch_one(&pool).await {
                Ok(row) => {
                    ...
                    match sqlx::query_scalar::<_, i64>(
                        "SELECT COUNT(*) FROM mig_topology.pools WHERE is_valid = true",
                    )
                    .fetch_one(&pool)
                    .await
                    {
                    ...
                    Ok(_) => {
                        ...
                        match redis_mgr.get_info().await {
                            Ok(info) => {
                                ...
                                if let Some(keyspace_line) =
                                    info.lines().find(|l| l.starts_with("keyspace"))
                                {
