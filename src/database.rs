...
pub async fn connect() -> Result<DbPool> {
    ...
    for attempt in 1..=max_attempts {
        ...
        {
            Ok(pool) => {
                log::info!("✅ Successfully connected to database (attempt {}/{}).", attempt, max_attempts);
...
async fn create_tables(tx: &mut sqlx::Transaction<'_, sqlx::Postgres>) -> Result<()> {
    ...
    sqlx::query(&format!("ALTER TABLE {}.tokens ALTER COLUMN symbol DROP NOT NULL", SCHEMA))
        .execute(tx.as_mut())
        .await
        .ok(); // Ignore error if column is already nullable

    sqlx::query(&format!("ALTER TABLE {}.tokens ALTER COLUMN decimals DROP NOT NULL", SCHEMA))
        .execute(tx.as_mut())
        .await
        .ok(); // Ignore error if column is already nullable

    // Add new columns if they don't exist (idempotent)
    sqlx::query(&format!("ALTER TABLE {}.tokens ADD COLUMN IF NOT EXISTS token_type VARCHAR(20)", SCHEMA))
        .execute(tx.as_mut())
        .await
        .ok();
    sqlx::query(&format!("ALTER TABLE {}.tokens ADD COLUMN IF NOT EXISTS oracle_source VARCHAR(30)", SCHEMA))
        .execute(tx.as_mut())
        .await
        .ok();
    sqlx::query(
        &format!("ALTER TABLE {}.tokens ADD COLUMN IF NOT EXISTS confidence_score DOUBLE PRECISION", SCHEMA),
    )
    .execute(tx.as_mut())
    .await
    .ok();
    sqlx::query(&format!("ALTER TABLE {}.tokens ADD COLUMN IF NOT EXISTS last_verified_block BIGINT", SCHEMA))
        .execute(tx.as_mut())
        .await
        .ok();

    // Drop any CHECK constraints on decimals column
    sqlx::query(&format!("ALTER TABLE {}.tokens DROP CONSTRAINT IF EXISTS tokens_decimals_check", SCHEMA))
        .execute(tx.as_mut())
        .await
    ...
    sqlx::query(&format!("ALTER TABLE {}.pools ADD COLUMN IF NOT EXISTS last_viable_at TIMESTAMPTZ", SCHEMA))
        .execute(tx.as_mut())
        .await
        .ok();
    sqlx::query(&format!("ALTER TABLE {}.pools ADD COLUMN IF NOT EXISTS last_viable_block BIGINT", SCHEMA))
        .execute(tx.as_mut())
        .await
        .ok();
    sqlx::query(&format!("ALTER TABLE {}.pools ADD COLUMN IF NOT EXISTS origin_dex VARCHAR(50)", SCHEMA))
        .execute(tx.as_mut())
        .await
        .ok();
    sqlx::query(&format!("ALTER TABLE {}.pools ADD COLUMN IF NOT EXISTS bytecode_hash VARCHAR(66)", SCHEMA))
        .execute(tx.as_mut())
        .await
        .ok();
    sqlx::query(&format!("ALTER TABLE {}.pools ADD COLUMN IF NOT EXISTS init_code_hash VARCHAR(66)", SCHEMA))
        .execute(tx.as_mut())
        .await
        .ok();
    ...
    sqlx::query(
        &format!("ALTER TABLE {}.pool_state_snapshots ADD COLUMN IF NOT EXISTS slot0_block BIGINT", SCHEMA),
    )
    .execute(tx.as_mut())
    .await
    .ok();
    sqlx::query(&format!("ALTER TABLE {}.pool_state_snapshots ADD COLUMN IF NOT EXISTS liquidity_block BIGINT", SCHEMA)).execute(tx.as_mut()).await.ok();
    sqlx::query(&format!("ALTER TABLE {}.pool_state_snapshots ADD COLUMN IF NOT EXISTS liquidity BIGINT", SCHEMA)).execute(tx.as_mut()).await.ok();
    sqlx::query(
        &format!("ALTER TABLE {}.pool_state_snapshots ADD COLUMN IF NOT EXISTS reserves_block BIGINT", SCHEMA),
    )
    .execute(tx.as_mut())
    .await
    .ok();
    ...
    // Create indexes
    sqlx::query(&format!("CREATE INDEX IF NOT EXISTS idx_pools_address ON {}.pools(address)", SCHEMA))
        .execute(tx.as_mut())
        .await?;
    sqlx::query(&format!("CREATE INDEX IF NOT EXISTS idx_pools_active ON {}.pools(is_active)", SCHEMA))
        .execute(tx.as_mut())
        .await?;
...
pub async fn get_dex_state(pool: &DbPool, dex_name: &str) -> Result<Option<DexState>> {
    let row = sqlx::query(
        &format!("SELECT dex, last_processed_block, mode FROM {}.dex_state WHERE dex = $1", SCHEMA),
    )
    .bind(dex_name)
    .fetch_optional(pool)
    .await?;
...
pub async fn batch_upsert_graph_weights(
    ...
    for chunk in weights.chunks(BATCH_CHUNK_SIZE) {
        ...
        let pool_addr_hex_vec: Vec<String> = chunk.iter()
...
pub async fn load_all_graph_weights(pool: &DbPool) -> Result<HashMap<Address, f64>> {
    let rows = sqlx::query(&format!("SELECT pool_address, weight FROM {}.graph_weights", SCHEMA))
        .fetch_all(pool)
        .await?;
...
pub async fn load_pools_by_addresses(pool: &DbPool, addresses: &[Address]) -> Result<Vec<PoolEnum>> {
    if addresses.is_empty() {
        return Ok(Vec::new());
    }
    
    // Convert addresses to hex strings for SQL IN clause
    let address_strings: Vec<String> = addresses.iter()
...
pub async fn load_valid_pools_by_liquidity_range(
    ...
    tracing::info!("📊 [OPTION 8] Query returned {} rows from database", total_rows);

    let mut pools = Vec::new();
    let mut skipped_count = 0;
    let mut skipped_dex: std::collections::HashMap<String, usize> = std::collections::HashMap::new();
    ...
    for row in rows {
        ...
        let token0 = match Address::from_str(&token0_str) {
            Ok(addr) => addr,
            Err(e) => {
                tracing::debug!("⚠️ Failed to parse token0 {} for pool {}: {}", token0_str, address_str, e);
                parse_errors += 1;
                skipped_count += 1;
                continue;
            }
        ...
        let token1 = match Address::from_str(&token1_str) {
            Ok(addr) => addr,
            Err(e) => {
                tracing::debug!("⚠️ Failed to parse token1 {} for pool {}: {}", token1_str, address_str, e);
                parse_errors += 1;
                skipped_count += 1;
                continue;
            }
        ...
        let reserve1 = reserve1_str
            .and_then(|s| s.parse::<u128>().ok())
            .unwrap_or(0);
        let mut liquidity = liquidity_opt
            .map(|l| l as u128)
            .unwrap_or(0);
        ...
        let pool_enum = match dex.as_str() {
            ...
            unsupported => {
                tracing::debug!("⚠️ Unsupported DEX type: {} for pool {}", unsupported, address_str);
                skipped_count += 1;
    ...
    if skipped_count > 0 {
        tracing::warn!("⚠️ [OPTION 8] Skipped {} pools ({} parse errors, {} unsupported DEX). Breakdown: {:?}", 
                      skipped_count, parse_errors, skipped_count - parse_errors, skipped_dex);
    }
...
pub async fn load_recently_discovered_pools(pool: &DbPool, seconds_ago: i64) -> Result<Vec<Address>> {
    let query = format!(
...
pub async fn set_pool_activity(pool: &DbPool, pool_address: &str, is_active: bool) -> Result<()> {
    sqlx::query(&format!("UPDATE {}.pools SET is_active = $1 WHERE address = $2", SCHEMA))
        .bind(is_active)
        .bind(pool_address)
        .execute(pool)
        .await?;
...
pub async fn get_valid_pools_count_per_dex(pool: &DbPool) -> Result<HashMap<String, i64>> {
    let rows = sqlx::query(
        &format!("SELECT dex, COUNT(*) as count FROM {}.pools WHERE is_valid = true GROUP BY dex", SCHEMA),
    )
    .fetch_all(pool)
    .await?;
...
async fn create_event_index_table_internal(tx: &mut sqlx::Transaction<'_, Postgres>) -> Result<()> {
    ...
    sqlx::query(
        &format!("CREATE INDEX IF NOT EXISTS idx_event_index_dex_block ON {}.event_index(dex, block_number)", SCHEMA)
    )
    .execute(tx.as_mut())
    .await?;
