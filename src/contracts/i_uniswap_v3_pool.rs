...
pub fn validate_slot0(sqrt_price_x96: U256, tick: i32, pool_address: &Address) -> Result<(), String> {
    // Validar sqrt_price_x96
    if sqrt_price_x96.is_zero() {
        return Err(format!("Invalid sqrt_price_x96: zero value for pool {}", pool_address));
    }
    
    // Validar tick
    if tick < MIN_TICK || tick > MAX_TICK {
        return Err(format!("Invalid tick: {} not in range [{}, {}] for pool {}", 
...
pub fn validate_fee_tier(fee: u32, pool_address: &Address) -> Result<(), String> {
    if !VALID_FEE_TIERS.contains(&fee) {
        return Err(format!("Invalid fee tier: {} not in {:?} for pool {}", 
...
pub fn validate_liquidity(liquidity: u128, pool_address: &Address) -> Result<(), String> {
    if liquidity == 0 {
        return Err(format!("Invalid liquidity: zero value for pool {}", pool_address));
    }
    Ok(())
}
