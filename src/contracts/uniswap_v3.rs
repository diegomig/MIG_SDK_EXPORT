...
pub async fn fetch_slot0_safe<M: ethers::providers::Middleware + 'static>(
    ...
    let (sqrt_price_x96, tick_raw, obs_idx, obs_card, obs_card_next, fee_protocol, unlocked) = raw_result;
    ...
    let tick = if tick_raw >= -8388608 && tick_raw <= 8388607 {
        ...
        eprintln!("⚠️  Warning: tick {} outside int24 range, clamping", tick_raw);
    ...
    <M as ethers::providers::Middleware>::Error: 'static,
{
    ...
    eprintln!("📦 Raw response ({} bytes): 0x{}", result.len(), hex::encode(&result));
...
pub fn validate_slot0(
    ...
    let max_sqrt_price = U256::from_dec_str(
...
pub async fn fetch_slot0_raw(
    ...
    println!("📦 Raw response ({} bytes): 0x{}", raw_result.len(), hex::encode(&raw_result));
