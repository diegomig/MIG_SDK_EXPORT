...
use crate::cache_state::{CacheValidationResult, StateCacheManager};
use crate::contracts::{uniswap_v3::UniswapV3Pool, IUniswapV2Pair};
...
use crate::{record_decision, record_error, record_phase_end, record_phase_start, record_rpc_call};
use dashmap::DashMap;
...
impl JitStateFetcher {
    ...
    pub fn with_hot_pool_manager(
        mut self,
        hot_pool_manager: Arc<crate::hot_pool_manager::HotPoolManager>,
    ) -> Self {
    ...
    fn get_pool_state_from_hot_pool_manager(
        &self,
        pool_address: &Address,
    ) -> Option<StateCacheCachedPoolState> {
    ...
    pub async fn pre_warm_cache_from_hot_pool_manager(&self, limit: usize) -> Result<usize> {
        ...
        let v3_pools: Vec<_> = hot_pool_manager
            .v3_hot_pools
            .iter()
            .take(limit)
        ...
        let v2_pools: Vec<_> = hot_pool_manager
            .v2_hot_pools
            .iter()
        ...
        info!(
            "✅ [JIT Cache Pre-warm] Pre-warmed {} pools from Hot Pool Manager in {:?}",
            pre_warmed_count,
            start.elapsed()
        );
    ...
    pub fn with_redis(
        mut self,
        redis_manager: Arc<tokio::sync::Mutex<crate::redis_manager::RedisManager>>,
    ) -> Self {
    ...
    pub async fn fetch_current_states(
        ...
        self.fetch_current_states_with_touched(pool_metadata, current_block, None)
            .await
    }
    ...
    pub async fn fetch_current_states_with_touched(
        ...
        for meta in pool_metadata {
            let is_touched = touched_pools
                .map(|tp| tp.contains(&meta.address))
                .unwrap_or(true);
            ...
            if let Some(cached) = self.state_cache.get(&meta.address) {
                ...
                let block_tolerance = if is_touched {
                    1
                } else {
                    cache_tolerance_blocks
                };

                // ✅ P0 OPTIMIZATION: TTL diferenciado completo con adaptive TTL
                let time_tolerance = if is_touched {
                    std::time::Duration::from_secs(
                        self.settings.performance.touched_pool_ttl_seconds,
                    )
                } else {
                        ...
                        && pool_weight > self.settings.performance.adaptive_ttl_weight_threshold
                    {
                        ...
                        std::time::Duration::from_secs(
                            self.settings.performance.untouched_pool_ttl_seconds / 2,
                        )
                    } else {
                        // Normal untouched pools: full TTL
                        std::time::Duration::from_secs(
                            self.settings.performance.untouched_pool_ttl_seconds,
                        )
                    }
                };
                ...
                if !cache_valid {
                    let reason = match validation_result {
                        ...
                        CacheValidationResult::InvalidBlockTolerance => {
                            format!(
                                "Block Diff {} > Tolerance {}",
                                blocks_since_cache, block_tolerance
                            )
                        }
                ...
                if cache_valid {
                    ...
                    if meta.pool_type == PoolType::V2 {
                        if let (Some(t0), Some(t1)) =
                            (cached_state.v2_token0, cached_state.v2_token1)
                        {
                    ...
                    match meta.pool_type {
                        ...
                        _ => {
                            ...
                            _cache_invalidation_reason =
                                Some("Missing required state in cache".to_string());
                        }
                    }
                }
            } else {
                ...
                if let Some(hpm_state) = self.get_pool_state_from_hot_pool_manager(&meta.address) {
                    debug!(
                        "✅ [JIT Cache] Pool {:?} found in Hot Pool Manager, using as fallback",
                        meta.address
                    );
                    ...
                    _cache_invalidation_reason =
                        Some("Pool not found in cache or Hot Pool Manager".to_string());
                    debug!(
                        "🔍 [JIT Cache] Pool {:?} not in cache or Hot Pool Manager (cache_size={})",
                        meta.address,
        ...
        if pools_to_fetch.len() <= 3 {
            info!(
                "🚀 [JIT Fast Path] {} pools to fetch (using fast path for edge cases)",
                pools_to_fetch.len()
            );
            ...
            if pools_to_fetch.len() > 1 {
                ...
                for pool in &pools_to_fetch {
                    match self
                        .fetch_single_pool_direct(pool, Arc::clone(&provider), Some(current_block))
                        .await
                    {
                        Ok(Some(fresh)) => {
                        ...
                        _ => {
                            ...
                            debug!(
                                "⚠️ [JIT Fast Path] Pool {:?} fetch failed, skipping",
                                pool.address
                            );
                        }
                    }
                ...
                for (addr, state) in &fast_path_results.v3_states {
                    if !cached_v3_states.contains_key(addr) {
                        let merkle_root =
                            Self::calculate_merkle_root(Some(state), None, current_block);
                        self.state_cache.insert(
                            *addr,
                            StateCacheCachedPoolState {
                ...
                for (addr, reserves) in &fast_path_results.v2_states {
                    if !cached_v2_states.contains_key(addr) {
                        let (token0, token1) = fast_path_results
                            .v2_tokens
                            .get(addr)
                            .copied()
                            .unwrap_or((Address::zero(), Address::zero()));
                        let merkle_root =
                            Self::calculate_merkle_root(None, Some(reserves), current_block);
                        self.state_cache.insert(
                            *addr,
                            StateCacheCachedPoolState {
                                v3_state: None,
                ...
                info!(
                    "✅ [JIT Fast Path] {} pools fetched in {}ms ({} V3, {} V2 states)",
                    pools_to_fetch.len(),
                    fast_path_results.fetch_duration.as_millis(),
                    fast_path_results.v3_states.len(),
                    fast_path_results.v2_states.len()
                );
            ...
            info!(
                "🚀 [JIT Fast Path] Single pool to fetch: {:?} (type: {:?})",
                pool.address, pool.pool_type
            );
        ...
        let mut build_calls_for_pools = |v3_pools: &[PoolMetadata],
                                         v2_pools: &[PoolMetadata]|
         -> (Vec<Call>, Vec<(&str, Address, PoolType)>) {
            ...
            for meta in v2_pools {
                ...
                if !cached_v2_tokens.contains_key(&meta.address) {
                    calls.push(Call {
                        target: meta.address,
                        call_data: v2_token0_calldata.clone(),
                    });
                    call_index_map.push(("v2_token0", meta.address, PoolType::V2));
                    calls.push(Call {
                        target: meta.address,
                        call_data: v2_token1_calldata.clone(),
                    });
        ...
        let (mut touched_calls, mut touched_call_index_map) =
            build_calls_for_pools(&touched_v3_pools, &touched_v2_pools);

        // ✅ P0 OPTIMIZATION: Build calls for untouched pools (low priority, can be larger batches)
        let (untouched_calls, untouched_call_index_map) =
            build_calls_for_pools(&untouched_v3_pools, &untouched_v2_pools);
        ...
        info!(
            "🔄 [JIT] Executing multicall with {} calls ({} V3 pools, {} V2 pools)",
        ...
        if untouched_count > 0 {
            ...
            let untouched_map_slice =
                &call_index_map[touched_count..touched_count + untouched_count];
        ...
        info!(
            "⏱️ [JIT] Using hard timeout: {}ms (RPC must respond within this limit)",
            HARD_RPC_TIMEOUT_MS
        );
        ...
        let payload_size_bytes = calls.iter().map(|call| call.call_data.len()).sum::<usize>();

        let pools_requested = total_v3 + total_v2;

        // ✅ SPRINT ESTABILIZACIÓN: Identificar pools solicitados para comparación
        let pools_requested_set: HashSet<Address> = touched_v3_pools
            .iter()
            .map(|m| m.address)
        ...
        for chunk_group in chunks[..chunks_to_process].chunks(max_parallelism) {
            ...
            for (chunk_idx, sub_calls, _sub_index, num_calls, is_touched_chunk) in chunk_group {
                ...
                let mc = Multicall::new(Arc::clone(&provider), self.multicall_address, batch_size);
                ...
                tasks.push(async move {
                    ...
                    let result =
                        tokio::time::timeout(hard_timeout_clone, mc.run(sub_calls_clone, None))
                            .await;
            ...
            let mut sorted_results: Vec<(
                usize,
                Result<
                    Result<Vec<ethers::types::Bytes>, anyhow::Error>,
                    tokio::time::error::Elapsed,
                >,
            )> = task_results.into_iter().collect();
            ...
            for (chunk_idx, task_result) in sorted_results {
                match task_result {
                    ...
                    Ok(Err(e)) => {
                        ...
                        warn!(
                            "⚠️ [JIT] Skipping failed chunk {} (no placeholders added)",
                            chunk_idx
                        );
        ...
        if results.len() != call_index_map.len() {
            warn!(
                "⚠️ [JIT] Multicall returned incomplete results: {} of {} calls",
                results.len(),
                call_index_map.len()
            );
        ...
        // ✅ SPRINT ESTABILIZACIÓN: Identificar pools solicitados para comparación
        let _pools_requested_set: HashSet<Address> = touched_v3_pools
            .iter()
            .map(|m| m.address)
            .chain(touched_v2_pools.iter().map(|m| m.address))
            .chain(untouched_v3_pools.iter().map(|m| m.address))
        ...
        let (
            ...
            .parse_multicall_results(&results, &call_index_map, &provider, &_pools_requested_set)
            .await?;
        ...
        for pool_addr in &failed_pools {
            ...
            if GLOBAL_CORRUPTION_TRACKER.try_register_multicall_failure(*pool_addr) {
                warn!(
                    "🔴 [JIT] Pool {:?} failed 3+ times in multicall, blacklisted for 24h",
                    pool_addr
                );
            }
        }
        ...
        if let Some(ref recorder) = self.flight_recorder {
            ...
            if pools_returned < pools_requested {
                record_error!(
                    ...
                    format!(
                        "Multicall returned {} pools but {} were requested",
                        pools_returned, pools_requested
                    ),
                    serde_json::json!({
        ...
        Ok(FreshPoolStates {
            ...
            v2_tokens,
            fetch_duration,
            total_calls: if pools_to_fetch.is_empty() {
                0
            } else {
                total_calls
            },
            successful_calls: if pools_to_fetch.is_empty() {
                cached_count
            } else {
                successful_calls
            },
    ...
    fn decode_slot0_flexible(
        ...
        Err(anyhow::anyhow!(
            "Cannot decode slot0: {} bytes (expected 224 for full ABI, 64+ for minimal)",
            bytes.len()
        ))
    }
    ...
    async fn parse_multicall_results(
            ...
            match (*call_type, *pool_type) {
                ...
                ("v3_liquidity", PoolType::V3) => {
                    match liquidity_fn.decode_output(&result_data.0) {
                            ...
                            if let Some(liquidity_u256) = decoded[0].clone().into_uint() {
                                ...
                                if let Some(liquidity) = liquidity_result {
                                    if liquidity > 0 {
                                        ...
                                        warn!(
                                            "⚠️ [JIT] V3 pool {:?} has zero liquidity",
                                            pool_addr
                                        );
                                    }
                                } else {
                        ...
                        Err(e) => {
                            warn!(
                                "⚠️ [JIT] V3 liquidity decoding failed for {:?}: {:?}",
                                pool_addr, e
                            );
                        }
                    }
                ...
                ("v2_reserves", PoolType::V2) => match reserves_fn.decode_output(&result_data.0) {
                    Ok(decoded) => {
                        if let (Some(reserve0), Some(reserve1)) = (
                            ...
                            if !reserve0.is_zero() && !reserve1.is_zero() {
                                ...
                                if !GLOBAL_CORRUPTION_TRACKER.is_corrupted(*pool_addr) {
                                    GLOBAL_CORRUPTION_TRACKER
                                        .mark_corrupted(*pool_addr, "jit_v2_zero_reserves");
                                }
                            }
                        }
                    }
                    Err(e) => {
                        warn!(
                            "⚠️ [JIT] V2 reserves decoding failed for {:?}: {:?}",
                            pool_addr, e
                        );
                        if !GLOBAL_CORRUPTION_TRACKER.is_corrupted(*pool_addr) {
                            GLOBAL_CORRUPTION_TRACKER
                                .mark_corrupted(*pool_addr, "jit_v2_reserves_decode_failed");
                        }
                    }
                },
                ("v2_token0", PoolType::V2) => match token0_fn.decode_output(&result_data.0) {
                    Ok(decoded) => {
                        if let Some(t0) = decoded.get(0).and_then(|t| t.clone().into_address()) {
                            let partial = v2_partial_tokens
                                .entry(*pool_addr)
                                .or_insert_with(PartialV2Tokens::default);
                            partial.token0 = Some(t0);
                            successful_calls += 1;
                        }
                    }
                    Err(e) => warn!(
                        "⚠️ [JIT] V2 token0 decoding failed for {:?}: {:?}",
                        pool_addr, e
                    ),
                },
                ("v2_token1", PoolType::V2) => match token1_fn.decode_output(&result_data.0) {
                    Ok(decoded) => {
                    ...
                    Err(e) => warn!(
                        "⚠️ [JIT] V2 token1 decoding failed for {:?}: {:?}",
                        pool_addr, e
                    ),
                },
                _ => {
                    warn!(
                        "⚠️ [JIT] Unknown call type: {} for pool {:?}",
                        call_type, pool_addr
                    );
                }
            }
        ...
        if v3_partial_used > 0 {
            info!(
                "📊 [JIT] Used {} partial V3 states with fallback liquidity",
                v3_partial_used
            );
        }
        ...
        for pool_addr in pools_requested {
            if !pools_successfully_parsed.contains(pool_addr) {
                ...
                warn!(
                    "🔴 [JIT] Pool {:?} failed in multicall (requested but not returned)",
                    pool_addr
                );
            }
        }
    ...
    async fn fetch_single_pool_direct(
        ...
        if GLOBAL_CORRUPTION_TRACKER.is_corrupted(pool_addr) {
            debug!(
                "⚠️ [JIT Fast Path] Pool {:?} is corrupted, skipping RPC call",
                pool_addr
            );
            return Ok(None);
        }
        ...
        match pool_meta.pool_type {
                ...
                match call_result {
                    Ok(Ok((slot0_result, liquidity_result))) => {
                        ...
                        if sqrt_price_x96.is_zero() || liquidity == 0 {
                            debug!(
                                "V3 pool {:?} has invalid state (zero price or liquidity)",
                                pool_addr
                            );
                            return Ok(None);
                        }
                    ...
                    Ok(Err(e)) => {
                        debug!(
                            "RPC call failed for V3 pool {:?}: {}. Pool likely doesn't exist.",
                            pool_addr, e
                        );
                        Ok(None)
                    }
                    Err(_) => {
                        debug!(
                            "Timeout fetching V3 pool {:?} (>200ms). Pool likely slow or inactive.",
                            pool_addr
                        );
                        Ok(None)
                    }
            ...
            PoolType::V2 => {
                ...
                match call_result {
                    ...
                    Ok(Err(e)) => {
                        debug!(
                            "RPC call failed for V2 pool {:?}: {}. Pool likely doesn't exist.",
                            pool_addr, e
                        );
                        Ok(None)
                    }
                    Err(_) => {
                        debug!(
                            "Timeout fetching V2 pool {:?} (>200ms). Pool likely slow or inactive.",
                            pool_addr
                        );
                        Ok(None)
                    }
...
mod tests {
    ...
    async fn jit_cache_only_returns_cached_states_when_total_calls_zero() {
        ...
        let rpc_pool =
            StdArc::new(RpcPool::new(StdArc::clone(&settings)).expect("RpcPool::new must succeed"));
        ...
        let v3_pool: Address = "0x00000000000000000000000000000000000000a1"
            .parse()
            .unwrap();
        let v2_pool: Address = "0x00000000000000000000000000000000000000b2"
            .parse()
            .unwrap();
        ...
        fetcher.state_cache.insert(
            ...
            StateCacheCachedPoolState {
                ...
                v2_token0: None,
                v2_token1: None,
                merkle_root: JitStateFetcher::calculate_merkle_root(
                    Some(&v3_state),
                    None,
                    current_block,
                ),
        ...
        let states = fetcher
            .fetch_current_states(&metas, current_block)
            .await
            .expect("fetch_current_states");
