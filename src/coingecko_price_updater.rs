...
impl CoinGeckoPriceUpdater {
    pub fn new(cache: SharedPriceCache) -> Self {
        ...
        token_map.insert(
            "0x35751007a407cA6FDEffe3b56c4e27da25D5c8D5"
                .parse()
                .unwrap(), // weETH
            ...
            match self.update_prices().await {
                ...
                Err(e) => {
                    ...
                    if consecutive_failures % 10 == 0 {
                        ...
                        warn!(
                            "⚠️ [CoinGecko #{}] Update failed (consecutive failures: {}): {}",
                    ...
                    if consecutive_failures >= 50 {
                        error!(
                            "🚨 CoinGecko updater has {} consecutive failures, may be rate limited",
    ...
    async fn update_prices(&self) -> Result<usize> {
        ...
        let mut coingecko_ids: Vec<String> = self
            .token_map
            .values()
            .cloned()
        ...
        for (address, coingecko_id) in &self.token_map {
            if let Some(token_price) = price_data.get(coingecko_id) {
                ...
                if price > 0.0 && price >= 0.0001 && price <= 1_000_000.0 {
                    ...
                    warn!(
                        "⚠️ CoinGecko: Invalid price for {} ({}): ${:.2}",
                        coingecko_id, address, price
                    );
                }
            }
        }
