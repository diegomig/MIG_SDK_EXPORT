...
use crate::pools::{Pool, CurveStableSwapPool};
...
impl DexAdapter for CurveAdapter {
    ...
    async fn discover_pools(
        ...
        if now - last < 180 { // 3 minutes = background_discoverer interval
        ...
        info!("📡 Querying AddressProvider at {} for MetaRegistry (ID={})", address_provider_addr, META_REGISTRY_ID);
        let meta_registry_addr = address_provider.get_address(U256::from(META_REGISTRY_ID)).call().await?;
        
        if meta_registry_addr == Address::zero() {
            return Err(anyhow::anyhow!("MetaRegistry not found in AddressProvider (ID={})", META_REGISTRY_ID));
        }
        ...
        let pool_count = meta_registry.pool_count().call().await?.as_u64().min(10_000); // Cap at 10k for safety
        ...
        let calls: Vec<_> = (0..pool_count).map(|i| {
            let call_data = meta_registry.pool_list(i.into()).calldata().unwrap();
            Call { target: meta_registry_addr, call_data }
        }).collect();

        let results = multicall.run(calls.clone(), None).await?;

        let pool_list_fn = meta_registry.abi().function("pool_list")?;
        let pool_addresses: Vec<Address> = results.into_iter().filter_map(|res| {
            pool_list_fn.decode_output(&res).ok()
                .and_then(|tokens| tokens[0].clone().into_address())
                .filter(|&addr| addr != Address::zero())
        }).collect();
        info!("📋 Discovered {} pool addresses from MetaRegistry", pool_addresses.len());

        // Batch call to get both tokens and fees
        let mut calls: Vec<_> = pool_addresses.iter().map(|&pool_address| {
            let call_data = meta_registry.get_underlying_coins(pool_address).calldata().unwrap();
            Call { target: meta_registry_addr, call_data }
        }).collect();
        
        // Add fee calls
        let fee_calls: Vec<_> = pool_addresses.iter().map(|&pool_address| {
        ...
        for (i, pool_address) in pool_addresses.iter().enumerate() {
            ...
            let tokens_result = if let Some(result_data) = results.get(i) {
                if result_data.is_empty() {
                    ...
                    match get_coins_fn.decode_output(result_data) {
                        Ok(decoded) => {
                            match decoded[0].clone() {
                                ethers::abi::Token::Array(arr) | ethers::abi::Token::FixedArray(arr) => {
                                    Some(arr.into_iter().filter_map(|t| t.into_address().filter(|&a| a != Address::zero())).collect::<Vec<Address>>())
                                }
                                _ => {
                                    decode_errors += 1;
                                    if decode_errors <= 5 {
                                        debug!("❌ Pool {}: decoded[0] is not an array. Type: {:?}", pool_address, decoded[0]);
                                    }
                                    None
                                }
                            }
                        ...
                        Err(e) => {
                            decode_errors += 1;
                            if decode_errors <= 3 {
                                debug!("❌ Failed to decode coins for pool {}: {}", pool_address, e);
                            }
                            None
                        }
                    }
                ...
                if !fee_data.is_empty() {
                    match get_fees_fn.decode_output(fee_data) {
                        Ok(decoded) => {
                            ...
                            if let Some(fee_u256) = decoded.get(0).and_then(|t| t.clone().into_uint()) {
                                let fee_bps = (fee_u256.as_u128() / 100_000_000) as u32; // Convert from 1e10 basis to bps
                                Some(fee_bps)
                            } else {
                                debug!("⚠️  Pool {}: fee decode returned unexpected type", pool_address);
            ...
            if let Some(tokens) = tokens_result {
                    ...
                    debug!("✅ Pool {} has {} tokens, fee: {:?} bps", pool_address, tokens.len(), fee);
                } else {
                    debug!("⚠️  Pool {} has only {} token(s), skipping", pool_address, tokens.len());
                }
            }
        }
        
        if decode_errors > 0 {
            warn!("⚠️  Failed to decode {} pool responses (empty or invalid ABI)", decode_errors);
        }
        if empty_results > 0 {
            warn!("⚠️  Received {} empty responses from MetaRegistry", empty_results);
        }

        info!("✅ Finished discovering from Curve MetaRegistry. Found {} valid pools.", discovered_pools.len());
        Ok(discovered_pools)
    }
    ...
    async fn fetch_pool_state(&self, pools: &[PoolMeta]) -> Result<Vec<Pool>> {
        ...
        loop {
            ...
            let address_provider_addr: Address = CURVE_ADDRESS_PROVIDER.parse()?;
            let address_provider = CurveAddressProvider::new(address_provider_addr, Arc::clone(&provider));
            let meta_registry_addr = address_provider.get_address(U256::from(META_REGISTRY_ID)).call().await?;
            let meta_registry = CurveMetaRegistry::new(meta_registry_addr, Arc::clone(&provider));

            let mut calls = Vec::new();
            for pool_meta in pools {
                calls.push(Call { target: pool_meta.address, call_data: ICurvePool::new(pool_meta.address, Arc::clone(&provider)).a().calldata().unwrap() });
                calls.push(Call { target: meta_registry_addr, call_data: meta_registry.get_underlying_balances(pool_meta.address).calldata().unwrap() });
                calls.push(Call { target: meta_registry_addr, call_data: meta_registry.get_underlying_coins(pool_meta.address).calldata().unwrap() });
                calls.push(Call { target: meta_registry_addr, call_data: meta_registry.get_fees(pool_meta.address).calldata().unwrap() });
            }
            ...
            match multicall.run(calls.clone(), None).await {
                Ok(results) => {
                    ...
                    let get_balances_fn = meta_registry.abi().function("get_underlying_balances")?;
                    let get_coins_fn = meta_registry.abi().function("get_underlying_coins")?;
                    let get_fees_fn = meta_registry.abi().function("get_fees")?;

                    let mut fetched_pools = Vec::new();
                    for (i, pool_meta) in pools.iter().enumerate() {
                        if let (Some(a_data), Some(balances_data), Some(coins_data), Some(fees_data)) = (results.get(i*4), results.get(i*4+1), results.get(i*4+2), results.get(i*4+3)) {
                            if let (Ok(a_decoded), Ok(balances_decoded), Ok(coins_decoded), Ok(fees_decoded)) =
                                (a_fn.decode_output(a_data), get_balances_fn.decode_output(balances_data), get_coins_fn.decode_output(coins_data), get_fees_fn.decode_output(fees_data)) {
                                if let (Some(a), Some(raw_balances_array), Some(raw_coins_array), Some(fees_array)) =
                                    (a_decoded[0].clone().into_uint(), balances_decoded[0].clone().into_array(), coins_decoded[0].clone().into_array(), fees_decoded[0].clone().into_array()) {
                                    ...
                                    for (j, coin_token) in raw_coins_array.into_iter().enumerate() {
                                        if let Some(coin_addr) = coin_token.into_address() {
                                            if coin_addr != Address::zero() {
                                                ...
                                                if let Some(bal) = raw_balances_array.get(j).and_then(|t| t.clone().into_uint()) {
                                                    balances.push(bal);
                                                }
                                            } else {
                                                break;
                                    ...
                                    let fee = fees_array.get(0).and_then(|f| f.clone().into_uint()).unwrap_or_default();

                                    if tokens.len() >= 2 {
                                        fetched_pools.push(Pool::CurveStableSwap(CurveStableSwapPool {
