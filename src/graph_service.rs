...
use crate::multicall::{self, Call, Multicall};
...
#[cfg(feature = "redis")]
use crate::redis_manager::{CachedPoolState, RedisManager};
use crate::{record_cache_event, record_phase_end, record_phase_start};
use anyhow::Result;
use dashmap::DashMap;
...
impl<M: Middleware + 'static> GraphService<M> {
    ...
    */
    ...
    pub async fn new(
        ...
        info!(
            "GraphService initialized with {} weights from database.",
            weights.len()
        );
        Ok(Self {
            rpc_pool,
    ...
    pub fn with_block_number_cache(
        mut self,
        block_number_cache: Arc<crate::block_number_cache::BlockNumberCache>,
    ) -> Self {
    ...
    pub async fn calculate_and_update_weights_for_pools(
        &self,
            ...
            && tokens_vec.len() > self.settings.performance.price_fetch_chunk_size
        {
            ...
            info!(
                "✅ [P1] Parallel price fetching: {} tokens split into {} chunks (chunk_size={})",
                tokens_vec.len(),
                chunks.len(),
                chunk_size
            );
                ...
                .await
            {
                Ok(mut prices) => {
                    if prices.is_empty() {
                        warn!(
                            "⚠️ Sequential price fetch returned empty prices for {} tokens",
                            tokens_vec.len()
                        );
                    } else {
                        // ✅ EMERGENCY PRICE REPAIR: Attempt to recover missing prices
                        let missing_tokens: Vec<Address> = tokens_vec
                            .iter()
                            .filter(|t| {
                                !prices.contains_key(t)
                                    || prices.get(t).map(|&p| p <= 0.0).unwrap_or(true)
                            })
                            .copied()
                            .collect();
                        ...
                        if !missing_tokens.is_empty() {
                            ...
                            let repair_tokens: Vec<Address> =
                                missing_tokens.iter().take(20).copied().collect();
                                ...
                                .await
                            {
                                Ok(repair_prices) => {
                                    let recovered =
                                        repair_prices.values().filter(|&&p| p > 0.0).count();
                                    prices.extend(repair_prices);
                                    info!(
                                        "✅ [Emergency Repair] Recovered {} prices (total: {})",
                                        recovered,
                                        prices.len()
                                    );
                                    ...
                                    if let Some(cache) = &self.shared_price_cache {
                                        let valid_repair: HashMap<Address, f64> = prices
                                            .iter()
                                            .filter(|(_, &p)| p > 0.0)
                                            .map(|(k, v)| (*k, *v))
                                            .collect();
                                        if !valid_repair.is_empty() {
                                            cache
                                                .update_batch(valid_repair, PriceSource::Chainlink);
                                        }
                                    }
                                }
                                Err(e) => {
                ...
                Err(e) => {
                    warn!(
                        "Failed to get prices batch: {}. Continuing with empty prices.",
                        e
                    );
                    HashMap::new()
                }
        ...
        let decimals_map = match self.get_decimals(&tokens_vec).await {
            ...
            Err(e) => {
                warn!(
                    "Failed to get decimals batch: {}. Continuing with defaults.",
                    e
                );
                HashMap::new()
            }
        ...
        for pool in pools_with_state {
            ...
            match self
                .calculate_liquidity_usd_with_cache(&pool, &prices_map, &decimals_map)
                .await
            {
                Ok(liquidity_usd) => {
                    let final_weight = if liquidity_usd > MAX_REASONABLE_WEIGHT_USD {
                        warn!(
                            "⚠️ Pool {} has extreme weight: ${:.2}. Filtering to 0.",
                ...
                Err(e) => {
                    warn!(
                        "Failed to calculate liquidity for pool {}: {}",
                        pool_address, e
                    );
                }
            }
        ...
        if !weights_to_update.is_empty() {
            ...
            match database::batch_upsert_graph_weights(&self.db_pool, &weights_to_update_clone)
                .await
            {
                Ok(_) => {
                    debug!(
                        "✅ [P1] Batch updated {} graph weights in database",
                        weights_to_update_clone.len()
                    );
                }
                Err(e) => {
                    warn!(
                        "⚠️ [P1] Batch update failed, falling back to individual updates: {}",
                        e
                    );
                    // Fallback to individual updates if batch fails
                    for (pool_address, weight, block) in weights_to_update_clone {
                        let pool_addr_hex = format!("{:#x}", pool_address);
                        if let Err(e) = database::upsert_graph_weight(
                            &self.db_pool,
                            &pool_addr_hex,
                            weight,
                            block,
                        ...
                        {
                            warn!(
                                "Failed to upsert graph weight for pool {}: {}",
                                pool_address, e
                            );
                        }
                    }
    ...
    pub async fn calculate_and_update_all_weights(&self) -> Result<()> {
        ...
        let tokens_vec: Vec<Address> = unique_tokens.into_iter().collect();
        info!(
            "📊 Pre-loading prices and decimals for {} unique tokens...",
            tokens_vec.len()
        );
            ...
            && tokens_vec.len() > self.settings.performance.price_fetch_chunk_size
        {
            ...
            if merged_prices.len() < tokens_vec.len() / 2 {
                let missing_tokens: Vec<Address> = tokens_vec
                    .iter()
                    .filter(|t| {
                        !merged_prices.contains_key(t)
                            || merged_prices.get(t).map(|&p| p <= 0.0).unwrap_or(true)
                    })
                    .copied()
                    .collect();
                ...
                if !missing_tokens.is_empty() {
                        ...
                        .await
                    {
                        Ok(repair_prices) => {
                            ...
                            // Update SharedPriceCache if available
                            if let Some(cache) = &self.shared_price_cache {
                                let valid_repair: HashMap<Address, f64> = merged_prices
                                    .iter()
                                    .filter(|(_, &p)| p > 0.0)
                                    .map(|(k, v)| (*k, *v))
                                    .collect();
        ...
        let decimals_map = match self.get_decimals(&tokens_vec).await {
            ...
            Err(e) => {
                warn!(
                    "Failed to get decimals batch: {}. Falling back to sequential.",
                    e
                );
                HashMap::new()
            }
        ...
        info!(
            "✅ Pre-loaded {} prices and {} decimals (out of {} unique tokens)",
            prices_map.len(),
            decimals_map.len(),
            tokens_vec.len()
        );
        ...
        if prices_map.len() < tokens_vec.len() / 10 {
            warn!(
                "⚠️  Only loaded {} prices for {} tokens. Most pools will have weight = 0.",
                prices_map.len(),
                tokens_vec.len()
            );
            ...
            let missing_tokens: Vec<Address> = tokens_vec
                .iter()
                .filter(|t| {
                    !prices_map.contains_key(t)
                        || prices_map.get(t).map(|&p| p <= 0.0).unwrap_or(true)
                })
                .copied()
                .collect();
            ...
            if !missing_tokens.is_empty() && missing_tokens.len() < tokens_vec.len() / 2 {
                    ...
                    .await
                {
                    Ok(repair_prices) => {
                        ...
                        // Update SharedPriceCache if available
                        if let Some(cache) = &self.shared_price_cache {
                            let valid_repair: HashMap<Address, f64> = prices_map
                                .iter()
                                .filter(|(_, &p)| p > 0.0)
                                .map(|(k, v)| (*k, *v))
                                .collect();
                    ...
                    Err(e) => {
                        warn!(
                            "⚠️ [Emergency Repair] Failed before weight calculation: {}",
                            e
                        );
                    }
                }
        ...
        if filtered_extreme_count > 0 {
            warn!(
                "🚨 Filtered {} pools with extreme weights (calculation errors)",
                filtered_extreme_count
            );
        }

        info!(
            "✅ Processed {} pools in a single batch, updated {} weights",
        ...
        if !weight_distribution.is_empty() {
            ...
            info!(
                "   Zero weight: {} ({:.1}%)",
                zero_weight_count,
                100.0 * zero_weight_count as f64 / total as f64
            );
            info!(
                "   Low weight (<1.0): {} ({:.1}%)",
                low_weight_count,
                100.0 * low_weight_count as f64 / total as f64
            );
            info!(
                "   Medium weight (1.0-10.0): {} ({:.1}%)",
                medium_weight_count,
                100.0 * medium_weight_count as f64 / total as f64
            );
            info!(
                "   High weight (>10.0): {} ({:.1}%)",
                high_weight_count,
                100.0 * high_weight_count as f64 / total as f64
            );
            info!("   Average: {:.2}", avg);
            info!(
                "   Percentiles - P25: {:.2}, P50: {:.2}, P75: {:.2}, P90: {:.2}, P95: {:.2}",
                p25, p50, p75, p90, p95
            );
            info!("   Max: {:.2}", max);
        ...
        info!(
            "Finished calculating and updating weights for {} pools.",
            self.weights.len()
        );
        ...
        if let Some(ref hot_pool_manager) = self.hot_pool_manager {
            if !calculated_weights_for_hot_pool.is_empty() {
                hot_pool_manager
                    .update_weights(calculated_weights_for_hot_pool.clone())
                    .await;
                info!(
                    "✅ Hot Pool Manager weights updated ({} pools)",
                    calculated_weights_for_hot_pool.len()
                );
            }
        }
    ...
    pub async fn fetch_pool_states(
        ...
        if let Some(ref redis_mgr) = self.redis_manager {
                    ...
                    Ok(Some(cached_state)) => {
                        ...
                        match self.cached_state_to_pool(&cached_state, &pool) {
                            Ok(converted_pool) => {
                                ...
                                if let Some(ref recorder) = self.flight_recorder {
                                    let current_block =
                                        if let Some(ref cache) = self.block_number_cache {
            ...
            if cache_hits > 0 {
                info!(
                    "✅ Redis cache hit: {}/{} pools ({:.1}%)",
                    cache_hits,
                    pools_len,
                    (cache_hits as f64 / pools_len as f64) * 100.0
                );
            }
            if cache_misses > 0 {
                info!(
                    "📊 Redis cache miss: {}/{} pools ({:.1}%)",
                    cache_misses,
                    pools_len,
                    (cache_misses as f64 / pools_len as f64) * 100.0
                );
            }
            ...
            let fetched_pools = self
                .fetch_pool_states_internal(pools_to_fetch, provider.clone())
                .await?;
            ...
            if !cached_states.is_empty() {
                ...
                match redis_guard.batch_cache_pool_states(&cached_states).await {
                    Ok(_) => {
                        info!(
                            "✅ Successfully cached {} pool states to Redis",
                            cached_states.len()
                        );
                    }
                    Err(e) => {
                        warn!(
                            "⚠️ Failed to cache {} pool states to Redis: {}",
                            cached_states.len(),
                            e
                        );
                    }
    ...
    fn cached_state_to_pool(&self, cached: &CachedPoolState, original: &Pool) -> Result<Pool> {
        ...
        match original {
            Pool::UniswapV2(p_orig) => {
                ...
                let reserve0_u256 = cached
                    .reserve0
                    .as_ref()
                    .and_then(|s| U256::from_dec_str(s).ok())
                    .unwrap_or_default();
                let reserve1_u256 = cached
                    .reserve1
                    .as_ref()
                    .and_then(|s| U256::from_dec_str(s).ok())
                    .unwrap_or_default();
            ...
            Pool::UniswapV3(p_orig) => {
                let sqrt_price_x96 = cached
                    .sqrt_price_x96
                    .as_ref()
                    .and_then(|s| U256::from_dec_str(s).ok())
                    .unwrap_or_default();
                let liquidity = cached
                    .liquidity
                    .as_ref()
    ...
    async fn fetch_pool_states_internal(
        ...
        if v2_pools.len() <= DIRECT_FALLBACK_V2_LIMIT
            && v3_pools.len() <= DIRECT_FALLBACK_V3_LIMIT
            && other_pools.is_empty()
        {
            debug!(
                "Using direct RPC fallback for {} V2 and {} V3 pools (small batch)",
                v2_pools.len(),
                v3_pools.len()
            );
            return self
                .fetch_pool_states_direct(v2_pools, v3_pools, other_pools, provider)
                .await;
        }
        ...
        if !v2_pools.is_empty() {
            ...
            match multicall.run(calls, None).await {
                Ok(results) => {
                    let pair_contract_dummy =
                        IUniswapV2Pair::new(Address::zero(), Arc::clone(&provider));
        ...
        if !v3_pools.is_empty() {
            ...
            match multicall.run(calls, None).await {
                Ok(results) => {
                    let pool_contract_dummy =
                        UniswapV3PoolContract::new(Address::zero(), Arc::clone(&provider));
                    ...
                    for (idx, (addr, mut pool)) in v3_pools.into_iter().enumerate() {
                        if idx < pool_indices.len() {
                            ...
                            if liquidity_idx < results.len() {
                                if let Ok(decoded) =
                                    liquidity_fn.decode_output(&results[liquidity_idx])
                                {
                                    if let Some(liq) = decoded[0]
                                        .clone()
                                        .into_uint()
                                        .and_then(|u| u.try_into().ok())
                                    {
    ...
    async fn fetch_pool_states_direct(
        ...
        for (addr, mut pool) in v3_pools {
            ...
            match pool_contract.liquidity().call().await {
                ...
                Err(e) => {
                    warn!(
                        "Direct fetch failed for V3 pool {:?} liquidity: {}",
                        addr, e
                    );
                }
            }
