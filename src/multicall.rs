...
impl<M: Middleware + 'static> Multicall<M> {
    pub fn new(provider: Arc<M>, multicall_address: Address, batch_size: usize) -> Self {
        ...
        if batch_size > 200 {
            log::warn!("⚠️ Batch size {} exceeds recommended maximum (200), capping to 200", batch_size);
        }
    ...
    pub async fn run(&self, calls: Vec<Call>, block: Option<BlockId>) -> Result<Vec<Bytes>> {
        ...
        let unique_call_vec: Vec<_> = unique_calls.into_keys().map(|(target, call_data)| Call { target, call_data }).collect();
        debug!("Multicall coalesced {} calls into {}", calls.len(), unique_call_vec.len());
        ...
        let final_results = original_indices.into_iter().map(|index| all_results_unique[index].clone()).collect();

        Ok(final_results)
    }

    /// ✅ FASE 2.1: Execute Multicall3 aggregate3 using direct ethers contract call
    async fn execute_multicall3(&self, calls: &[Call], block: Option<BlockId>) -> Result<Vec<Bytes>> {
        ...
        let results_array = decoded.into_iter().next()
