...
impl BlockNumberCache {
    ...
    pub async fn get_current_block(&self) -> Result<u64> {
        ...
        match self.provider.get_block_number().await {
            ...
            Err(e) => {
                ...
                if cached > 0 {
                    warn!(
                        "BlockNumberCache: Failed to fetch block number, using cached value {}: {}",
                        cached, e
                    );
                    Ok(cached)
                } else {
                    Err(anyhow::anyhow!(
                        "Failed to fetch block number and no cache available: {}",
                        e
                    ))
                }
            }
    ...
    pub async fn force_update(&self) -> Result<u64> {
        ...
        match self.provider.get_block_number().await {
            ...
            Err(e) => {
                ...
                if cached > 0 {
                    warn!(
                        "BlockNumberCache: Force update failed, using cached value {}: {}",
                        cached, e
                    );
                    Ok(cached)
                } else {
                    Err(anyhow::anyhow!(
                        "Failed to force update block number and no cache available: {}",
                        e
                    ))
                }
            }
    ...
    pub fn update_from_external(&self, block_number: u64) {
        if block_number > 0 {
            ...
            debug!(
                "BlockNumberCache: Updated from external source to block {}",
                block_number
            );
        }
    }
