...
impl FlightRecorder {
    ...
    pub fn record(&self, event: FlightEvent) {
        ...
        let event_with_ts = match event {
            FlightEvent::BlockStart { ts: 0, block } => FlightEvent::BlockStart {
                ts: self.timestamp(),
                block,
            },
            FlightEvent::BlockEnd {
                ts: 0,
                block,
                duration_ms,
                routes_generated,
        ...
        self.serialization_time_ns
            .fetch_add(serialization_duration.as_nanos() as u64, Ordering::Relaxed);
...
pub async fn flight_recorder_writer(
    ...
    if let Some(parent) = std::path::Path::new(&output_file).parent() {
        match std::fs::create_dir_all(parent) {
            ...
            Err(e) => {
                eprintln!(
                    "❌ Flight Recorder: Failed to create directory {:?}: {}",
                    parent, e
                );
                return Err(e);
            }
    ...
    let file = match File::create(&output_file).await {
        Ok(f) => {
            println!(
                "✅ Flight Recorder: File created successfully: {}",
                output_file
            );
            f
        }
        Err(e) => {
            eprintln!(
                "❌ Flight Recorder: Failed to create file {}: {}",
                output_file, e
            );
            return Err(e);
        }
    ...
    writer.flush().await?;
    println!(
        "✅ Flight Recorder saved {} events to {}",
        count, output_file
    );
...
macro_rules! record_phase_start {
    ($recorder:expr, $phase:expr) => {
        $crate::flight_recorder::FlightRecorderHelper::record_event(
            &$recorder,
            $crate::flight_recorder::FlightEvent::PhaseStart {
                ts: 0,
...
macro_rules! record_phase_end {
    ($recorder:expr, $phase:expr, $start:expr, $result:expr) => {{
        ...
        $crate::flight_recorder::FlightRecorderHelper::record_event(
            &$recorder,
            $crate::flight_recorder::FlightEvent::PhaseEnd {
                ts: 0,
                phase: $phase.to_string(),
                duration_ms,
                result: $result,
...
macro_rules! record_decision {
    ($recorder:expr, $component:expr, $action:expr, $reason:expr) => {
        $crate::flight_recorder::FlightRecorderHelper::record_event(
            &$recorder,
            $crate::flight_recorder::FlightEvent::Decision {
                ts: 0,
...
macro_rules! record_coordination {
    ($recorder:expr, $what:expr, $details:expr) => {
        $crate::flight_recorder::FlightRecorderHelper::record_event(
            &$recorder,
            $crate::flight_recorder::FlightEvent::Coordination {
                ts: 0,
                what: $what.to_string(),
                details: $details,
                block: None,
...
macro_rules! record_rpc_call {
    ($recorder:expr, $endpoint:expr, $method:expr, $start:expr, $success:expr) => {{
        ...
        $crate::flight_recorder::FlightRecorderHelper::record_event(
            &$recorder,
            $crate::flight_recorder::FlightEvent::RpcCall {
                ts: 0,
...
macro_rules! record_error {
    ($recorder:expr, $component:expr, $error_type:expr, $message:expr) => {
        $crate::flight_recorder::FlightRecorderHelper::record_event(
            &$recorder,
            $crate::flight_recorder::FlightEvent::Error {
                ts: 0,
                component: $component.to_string(),
...
macro_rules! record_block_skipped {
    ($recorder:expr, $block:expr, $reason:expr, $last_processed:expr) => {
        ...
        $crate::flight_recorder::FlightRecorderHelper::record_event(
            &$recorder,
            $crate::flight_recorder::FlightEvent::BlockSkipped {
                ts: 0,
                block: $block,
                reason: $reason.to_string(),
...
macro_rules! record_block_gap {
    ($recorder:expr, $current_block:expr, $last_processed:expr, $threshold:expr) => {
        ...
        $crate::flight_recorder::FlightRecorderHelper::record_event(
            &$recorder,
            $crate::flight_recorder::FlightEvent::BlockGap {
                ts: 0,
...
macro_rules! record_block_start {
    ($recorder:expr, $block:expr) => {
        $crate::flight_recorder::FlightRecorderHelper::record_event(
            &$recorder,
            $crate::flight_recorder::FlightEvent::BlockStart {
                ts: 0,
                block: $block,
            },
        );
...
macro_rules! record_block_end {
    ($recorder:expr, $block:expr, $start:expr, $routes_generated:expr, $routes_filtered:expr, $opportunities:expr, $gas_saved_l1:expr, $gas_saved_l2:expr) => {{
        ...
        $crate::flight_recorder::FlightRecorderHelper::record_event(
            &$recorder,
            $crate::flight_recorder::FlightEvent::BlockEnd {
                ts: 0,
                block: $block,
                duration_ms,
...
macro_rules! record_cache_event {
    ($recorder:expr, $cache_type:expr, $event_type:expr, $key:expr, $block:expr) => {
        $crate::flight_recorder::FlightRecorderHelper::record_event(
            &$recorder,
            $crate::flight_recorder::FlightEvent::CacheEvent {
                ts: 0,
