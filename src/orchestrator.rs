...
use crate::{
    ...
    record_phase_start, record_phase_end, record_block_start, record_block_end,
};
...
impl<M: Middleware + 'static> Orchestrator<M> {
    ...
    pub async fn check_pools_activity(&self) -> Result<()> {
        ...
        info!("Checking activity for {} active pools...", active_pools.len());
        ...
        let current_block = if let Some(ref cache) = self.block_number_cache {
            ...
            let (provider_for_block, _permit, endpoint) = self.rpc_pool.get_next_provider_with_endpoint().await?;
            self.rpc_pool.get_block_number_with_recording(&provider_for_block, &endpoint).await?.as_u64()
        };
        ...
        let pools_to_check = if let Some(ref block_parser) = self.block_parser {
            ...
            let active_pool_addresses: std::collections::HashSet<Address> = active_pools.iter()
                .map(|p| p.address())
                .collect();
            
            // Filtrar: solo pools que están en active_pools Y fueron tocados
            let filtered_pools: Vec<Pool> = active_pools.into_iter()
                .filter(|p| touched_pools.contains(&p.address()) || active_pool_addresses.len() < 100)
                .collect();
            ...
            info!("🚀 [BlockFilter] Filtered {} pools to {} pools ({}% reduction)", 
                  active_pool_addresses.len(), filtered_pools.len(), reduction_pct);
            #[cfg(feature = "observability")]
            {
                metrics::histogram!("discoverer_pools_filtered_reduction_pct", reduction_pct);
                metrics::histogram!("discoverer_pools_before_filtering", active_pool_addresses.len() as f64);
                metrics::histogram!("discoverer_pools_after_filtering", filtered_pools.len() as f64);
            }
        ...
        let pools_with_state = self.fetch_pool_states_batch(&pools_to_check, provider).await?;
        ...
        let token_prices = if !unique_tokens_vec.is_empty() {
            match self.price_feed.get_usd_prices_batch(&unique_tokens_vec, None).await {
                Ok(prices) => {
                ...
                Err(e) => {
                    warn!("⚠️ [Batch Prices] Failed to fetch prices: {:?}, using empty map", e);
        ...
        for pool in pools_with_state {
            let (usd_value, threshold) = match &pool {
                Pool::UniswapV2(p) => {
                    ...
                    let decimals0 = self.cache_manager.token_decimals_cache.get(&p.token0).map(|d| *d).unwrap_or(18);
                    let reserve0_f = ethers::utils::format_units(p.reserve0, decimals0 as u32).unwrap_or("0.0".to_string()).parse::<f64>().unwrap_or(0.0);
                    let value0 = reserve0_f * price0;

                    let price1 = token_prices.get(&p.token1).copied().unwrap_or_default();
                    let decimals1 = self.cache_manager.token_decimals_cache.get(&p.token1).map(|d| *d).unwrap_or(18);
                    let reserve1_f = ethers::utils::format_units(p.reserve1, decimals1 as u32).unwrap_or("0.0".to_string()).parse::<f64>().unwrap_or(0.0);
                ...
                Pool::BalancerWeighted(p) => {
                    ...
                    for (i, token) in p.tokens.iter().enumerate() {
                        if let Some(balance) = p.balances.get(i) {
                            ...
                            let decimals = self.cache_manager.token_decimals_cache.get(token).map(|d| *d).unwrap_or(18);
                            let balance_f = ethers::utils::format_units(*balance, decimals as u32).unwrap_or("0.0".to_string()).parse::<f64>().unwrap_or(0.0);
            ...
            if usd_value > 0.0 && usd_value < threshold {
                warn!("Pool {:?} is now inactive. Liquidity: ${:.2}", pool.address(), usd_value);
                ...
                if let Some(ref db_writer) = self.db_writer {
                    ...
                    database::set_pool_activity(&self.db_pool, &pool.address().to_string(), false).await?;
                }
                ...
                if let Some(ref db_writer) = self.db_writer {
                    ...
                    database::set_pool_activity(&self.db_pool, &pool.address().to_string(), true).await?;
                }
            }
        ...
        if !inactive_sample.is_empty() {
            info!("Found {} inactive valid pools to check, verifying liquidity...", inactive_sample.len());
            let (provider2, _permit2) = self.rpc_pool.get_next_provider().await?;
            let inactive_with_state = self.fetch_pool_states_batch(&inactive_sample, provider2).await?;
            let mut reactivated_count = 0;
            
            // 🚀 RPC OPTIMIZATION: Batch de precios para pools inactivos
            let mut unique_tokens_inactive: std::collections::HashSet<Address> = std::collections::HashSet::new();
            ...
            let unique_tokens_inactive_vec: Vec<Address> = unique_tokens_inactive.iter().copied().collect();
            let token_prices_inactive = if !unique_tokens_inactive_vec.is_empty() {
                match self.price_feed.get_usd_prices_batch(&unique_tokens_inactive_vec, None).await {
                    Ok(prices) => prices,
            ...
            for pool in inactive_with_state {
                let (usd_value, threshold) = match &pool {
                    Pool::UniswapV2(p) => {
                        let price0 = token_prices_inactive.get(&p.token0).copied().unwrap_or_default();
                        let decimals0 = self.cache_manager.token_decimals_cache.get(&p.token0).map(|d| *d).unwrap_or(18);
                        let reserve0_f = ethers::utils::format_units(p.reserve0, decimals0 as u32).unwrap_or("0.0".to_string()).parse::<f64>().unwrap_or(0.0);
                        let value0 = reserve0_f * price0;
                        
                        let price1 = token_prices_inactive.get(&p.token1).copied().unwrap_or_default();
                        let decimals1 = self.cache_manager.token_decimals_cache.get(&p.token1).map(|d| *d).unwrap_or(18);
                        let reserve1_f = ethers::utils::format_units(p.reserve1, decimals1 as u32).unwrap_or("0.0".to_string()).parse::<f64>().unwrap_or(0.0);
                        let value1 = reserve1_f * price1;
                ...
                if usd_value >= threshold {
                    info!("Reactivating pool {:?} with liquidity ${:.2}", pool.address(), usd_value);
                    // ✅ BATCH WRITES: Use PostgresAsyncWriter
                    if let Some(ref db_writer) = self.db_writer {
                        if let Err(e) = db_writer.set_pool_activity(pool.address(), true) {
                            error!("Failed to queue set pool activity: {}", e);
            ...
            if reactivated_count > 0 {
                info!("✅ Reactivated {} pools that have sufficient liquidity", reactivated_count);
            }
        }
    ...
    async fn fetch_pool_states_batch(&self, pools: &[Pool], provider: Arc<Provider<Http>>) -> Result<Vec<Pool>> {
        let multicall_address = self.settings.contracts.factories.multicall.parse::<Address>()
        ...
        if !v3_pools.is_empty() {
            ...
            // Ejecutar en batches
            let mut v3_partial: HashMap<Address, (Option<U256>, Option<i64>, Option<u128>)> = HashMap::new();
    ...
    #[allow(dead_code)]
    async fn fetch_pool_states(&self, pools: Vec<Pool>, provider: Arc<Provider<Http>>) -> Result<Vec<Pool>> {
    ...
    pub async fn run_discovery_cycle(&self) -> Result<()> {
        ...
        for adapter in &self.adapters {
            ...
            let current_block = if let Some(ref cache) = self.block_number_cache {
                ...
                self.rpc_pool.get_block_number_with_recording(&provider, &endpoint).await?.as_u64()
            };
            let mut state = match database::get_dex_state(&self.db_pool, adapter_name).await {
                Ok(Some(s)) => {
                    eprintln!("🔍 [DEBUG] Found dex_state: last_processed_block={}, mode={}", s.last_processed_block, s.mode);
                    s
                }
                Ok(None) => {
            ...
            if adapter_name == "Curve" {
                ...
                let pools_meta = adapter.discover_pools(current_block, current_block, 0, 0).await?;
                info!("Static discovery for Curve: found {} pools (one-time call)", pools_meta.len());
                ...
                for (pool_meta, result) in validation_results {
                    ...
                    if let Some(ref db_writer) = self.db_writer {
                        ...
                        if let Err(e) = database::upsert_pool(&self.db_pool, &pool_meta, current_block, is_structurally_valid, true).await {
            ...
            let blocks_to_process = if from >= to || (to - from) > self.settings.discovery.initial_sync_blocks {
            ...
            if blocks_to_process.0 >= blocks_to_process.1 {
                info!("No new blocks to process for {} (last_processed: {}, current: {})", adapter_name, from, current_block);
            ...
            info!("Performing FORWARD sync for {} from block {} to {} (range: {} blocks)", 
                  adapter_name, blocks_to_process.0, blocks_to_process.1, blocks_to_process.1.saturating_sub(blocks_to_process.0));
            let (from_block, to_block, backwards) = (blocks_to_process.0, blocks_to_process.1, false);
            ...
            while (backwards && current_pos >= from_block) || (!backwards && current_pos <= to_block) {
                iterations += 1;
                ...
                let (chunk_start, chunk_end) = if backwards {
                    (current_pos.saturating_sub(chunk_size - 1).max(from_block), current_pos)
                } else {
                ...
                if chunk_start > chunk_end {
                    warn!("⚠️ Invalid chunk range: start {} > end {} for {}. Breaking loop.", 
                          chunk_start, chunk_end, adapter_name);
                    break;
                }

                info!("Processing chunk {}-{} for {} (reverse: {}, iteration: {})", 
                ...
                let pools_meta = match adapter.discover_pools(chunk_start, chunk_end, chunk_size, max_concurrency).await {
                    Ok(pools) => pools,
                    Err(e) => {
                        error!("Failed to discover pools for {} in chunk {}-{}: {}", adapter_name, chunk_start, chunk_end, e);
                        if backwards {
                            if chunk_start == from_block { break; }
                ...
                info!("Discovered {} potential pools for {} in chunk {}-{}", pools_meta.len(), adapter_name, chunk_start, chunk_end);
                ...
                if !valid_pools_meta.is_empty() {
                    let pools_with_state = adapter.fetch_pool_state(&valid_pools_meta).await.unwrap_or_default();
                    ...
                    let mut unique_tokens_discovery: std::collections::HashSet<Address> = std::collections::HashSet::new();
                    for pool in &pools_with_state {
                        match pool {
                            Pool::UniswapV2(p) => {
                    ...
                    let unique_tokens_discovery_vec: Vec<Address> = unique_tokens_discovery.iter().copied().collect();
                    let token_prices_discovery = if !unique_tokens_discovery_vec.is_empty() {
                        match self.price_feed.get_usd_prices_batch(&unique_tokens_discovery_vec, None).await {
                            Ok(prices) => prices,
                            Err(_) => std::collections::HashMap::new()
                        }
                    } else {
                    ...
                    for pool in pools_with_state {
                        let (usd_value, threshold) = match &pool {
                             Pool::UniswapV2(p) => {
                                let price0 = token_prices_discovery.get(&p.token0).copied().unwrap_or_default();
                                let decimals0 = self.cache_manager.token_decimals_cache.get(&p.token0).map(|d| *d).unwrap_or(18);
                                let reserve0_f = ethers::utils::format_units(p.reserve0, decimals0 as u32).unwrap_or("0.0".to_string()).parse::<f64>().unwrap_or(0.0);
                                let value0 = reserve0_f * price0;
                                let price1 = token_prices_discovery.get(&p.token1).copied().unwrap_or_default();
                                let decimals1 = self.cache_manager.token_decimals_cache.get(&p.token1).map(|d| *d).unwrap_or(18);
                                let reserve1_f = ethers::utils::format_units(p.reserve1, decimals1 as u32).unwrap_or("0.0".to_string()).parse::<f64>().unwrap_or(0.0);
                                let value1 = reserve1_f * price1;
                                (value0 + value1, min_v2_reserve_usd)
                            ...
                            Pool::UniswapV3(p) => {
                                let price0 = token_prices_discovery.get(&p.token0).copied().unwrap_or_default();
                                let decimals0 = self.cache_manager.token_decimals_cache.get(&p.token0).map(|d| *d).unwrap_or(18);
                                let liquidity_f = ethers::utils::format_units(p.liquidity, decimals0 as u32).unwrap_or("0.0".to_string()).parse::<f64>().unwrap_or(0.0);
                        ...
                        if is_active { active_pools_count += 1; }
                    ...
                    if !activity_updates.is_empty() {
                        if let Some(ref db_writer) = self.db_writer {
                            if let Err(e) = db_writer.batch_set_pool_activity(activity_updates.clone()) {
                                error!("Failed to queue batch set pool activity: {}", e);
                            }
                        } else {
                            // Fallback to individual writes
                            for (address, is_active) in activity_updates {
                                if let Err(e) = database::set_pool_activity(&self.db_pool, &format!("{:?}", address), is_active).await {
                ...
                let (gas_saved_l1, gas_saved_l2) = if pools_processed_count > 0 {
                    ...
                    let num_multicall_batches = ((num_individual_calls + batch_size - 1) / batch_size).max(1); // ceil division
                    ...
                    let gas_multicall = num_multicall_batches * 50000u64 + num_individual_calls * 100u64; // Base 50k + 100 per call
                ...
                if backwards {
                    if chunk_start == from_block { break; } // Reached end of reverse batch
                    current_pos = chunk_start.saturating_sub(1);
                } else {
            ...
            if state.mode == "flash_pending" {
                info!("Initial forward sync for {} complete. Switching to reverse sync mode.", adapter_name);
        ...
        if let Some(ref recorder) = self.flight_recorder {
            ...
            record_phase_end!(recorder, "discovery_cycle", discovery_start, serde_json::json!({
